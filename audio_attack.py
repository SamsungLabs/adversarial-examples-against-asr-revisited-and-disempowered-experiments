"""
    ...
    All rights reserved.
    Modified by: Samsung R&D Poland
"""
import os, sys
import numpy as np
import sys
...
def mutation(x, eps_limit, endianness):
    ba = bytearray(x)
    step = 2
    #if pbs == 8:
    #    step = 1
    for i in range(header_len, len(x), step):
        ...
        if np.random.random() < mutation_p:
            int_x = int.from_bytes(ba[i:i+2], byteorder=endianness, signed=True)
            new_int_x = min(data_max, max(data_min, int_x + np.random.choice(range(-eps_limit, eps_limit))))
            new_bytes = int(new_int_x).to_bytes(2, byteorder=endianness, signed=True)
            ba[i] = new_bytes[0]
            ba[i+1] = new_bytes[1]
    return bytes(ba)
...
def generate_attack(x_orig, target, limit, sess, input_node,
    output_node, max_iters, eps_limit=256, verbose=False, endianness='big'):
    pop_size = 20
    elite_size = 2
    temp = 0.01
    ...
    for idx in range(max_iters):
        ...
        initial_pop = elite_set + [mutation(child, eps_limit, endianness) for child in child_set]
    return top_attack, -1
